<x-admin-layout>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="long_description" id="long_description" rows="6" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary">{{ old('long_description', $product->long_description) }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="variants" id="variants" rows="4"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary">{{ old('variants', is_string($product->variants) ? $product->variants : json_encode($product->variants)) }}</textarea>
    ...
    @if($product->image_url)
        <div class="mt-6 bg-white rounded-lg shadow-md overflow-hidden">
            <div class="p-6">
                <h3 class="text-lg font-semibold mb-4">Current Product Image</h3>
                <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="max-w-full h-auto max-h-64 object-contain">
            </div>
        </div>
    @endif
</x-admin-layout>
